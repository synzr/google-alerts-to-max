...
class AttemptExecutor:
    ...
    def try_execute(
        ...
        :param callable: Вызываемое (функция/ламбада)
        ...
        :param try_wait_secs: Время ожидания перед следующей попыткой (в секундах) (по умолчанию: одна секунда)
        :param success_condition: Функция, которая получает результат вызываемого и возвращает булевое значение
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                future = self.__pool.submit(callable)
                ...
                if success_condition and not success_condition(result):
                    raise SuccessConditionFailedException(
                        "Провалено исключение успешного выполнения",
                        result=result
                    )
                ...
                if attempt < attempts:
                    ...
                    jitter = random.uniform(0, backoff * .3)
